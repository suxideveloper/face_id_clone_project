...
class AttendanceDatabase:
    ...
    def get_worker_history(self, worker_id: str, limit: int = 30) -> list:
        ...
        with SessionLocal() as db:
            rows = (
                ...
                .order_by(Attendance.date.desc(), func.coalesce(Attendance.check_out_time, Attendance.check_in_time).desc())
                .limit(limit)
                .all()
            )
    ...
    def get_all_records(self, start_date=None, end_date=None) -> list:
        ...
        with SessionLocal() as db:
            ...
            rows = q.order_by(Attendance.date.desc(), func.coalesce(Attendance.check_out_time, Attendance.check_in_time).desc()).all()
            return [self._row_to_dict(r) for r in rows]
